...
export function RecurringTaskCard({
  ...
  const handleCompleteClick = () => {
    if (todaysEntry?.completed) {
      // If already completed, just toggle completion status directly
      onComplete({
        ...todaysEntry,
        completed: false
      });
    } else {
      // If not completed, open dialog for adding details
    ...
    <>
      <Card className={`w-full bg-zinc-900/90 border-zinc-800 rounded-2xl shadow-md transition-transform hover:shadow-lg hover:translate-y-[-2px] animate-fade-in ${todaysEntry?.completed ? 'opacity-70' : ''}`}>
        ...
        <CardContent className="py-0">
          ...
          {/* Show details if today's entry has them */}
          {todaysEntry?.completed && todaysEntry.details && (
      ...
      {/* Dialog for adding details during task completion */}
      <Dialog open={isCompletionDialogOpen} onOpenChange={setIsCompletionDialogOpen}>
        <DialogContent className="bg-zinc-900 border border-zinc-700">
          ...
          <DialogFooter className="flex gap-2">
            <Button variant="outline" onClick={() => setIsCompletionDialogOpen(false)}>Cancelar</Button>
            <Button onClick={handleComplete} className="bg-cyan-700 hover:bg-cyan-600">
